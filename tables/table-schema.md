...
| DFPVU and Ara VMFPU4 implementation cost | Make the arithmetic-boundary provenance and comparison limits auditable | DFPVU `PvuTop`; four Ara `vmfpu` instances | target period, mapped cells/area, core/die area, utilization, physical-stage status, and prohibited PPA ratios | `docs/result/dfpvu_openroad_ppa_45ns.md`; `docs/result/ara_vmfpu4_openroad_compute_cost_45ns.md`; `docs/result/dfpvu_ara_vmfpu4_openroad_comparison_45ns.md` |

All RTL and physical values are single-run observations. No dispersion statistic is defined.
