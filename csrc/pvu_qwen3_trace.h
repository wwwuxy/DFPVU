...
inline size_t parse_nonnegative_argument(const char* argument,
...
inline LlmTrace load_llm_trace(const std::string& root) {
  ...
  LlmTrace trace;
  trace.format_version = version;
  trace.model =
      require_string(require_member(metadata, "model", "root"), "model");
  if (trace.model.empty()) {
    throw std::runtime_error(
        "metadata.json: model must not be empty");
  }
  trace.prompt =
      require_string(require_member(metadata, "prompt", "root"), "prompt");
  if (version == 2) {
    trace.profile = require_string(
        require_member(metadata, "profile", "root"), "profile");
    if (trace.profile.empty()) {
      throw std::runtime_error("metadata.json: profile must not be empty");
    }
    trace.output_semantics = require_string(
        require_member(metadata, "output_semantics", "root"),
        "output_semantics");
    if (trace.output_semantics != "linear-no-bias") {
      throw std::runtime_error(
          "metadata.json: output_semantics must be linear-no-bias");
    }
  } else {
    trace.profile = "legacy-qwen3-v1";
    trace.output_semantics = "linear-no-bias";
  }
  ...
  const auto& module_values =
      require_type(require_member(metadata, "modules", "root"),
          ...
          .array;
  if (module_values.empty()) {
    throw std::runtime_error("metadata.json: modules must not be empty");
  }
  ...
  for (size_t index = 0; index < module_values.size(); ++index) {
    ...
    const bool has_bias = require_bool(
        require_member(descriptor, "has_bias", context),
        context + ".has_bias");
    if (version == 1 && has_bias) {
      throw std::runtime_error("metadata.json: " + context +
                               " v1 trace declares an unsupported bias");
    }
    ...
    if ((version == 1 && input_shape[0] != trace.input_token_ids.size()) ||
...
inline LlmSelection parse_llm_selection(int argc, char** argv) {
  if (argc < 1 || argc > 5) {
    throw std::runtime_error(
        "usage: llm-workload [trace-root [M [N [K]]]]");
  }
  ...
  if (argc >= 3) {
    selection.m = qwen3_trace_detail::parse_nonnegative_argument(argv[2], "M");
  }
  if (argc >= 4) {
    selection.n = qwen3_trace_detail::parse_nonnegative_argument(argv[3], "N");
  }

  if (argc >= 5) {
    selection.k = qwen3_trace_detail::parse_nonnegative_argument(argv[4], "K");
  }
  const LlmTrace trace = load_llm_trace(selection.trace_root);
  for (const auto& entry : trace.modules) {
    const LinearModuleTrace& module = entry.second;
    if (selection.m > module.input.shape[0]) {
      throw std::runtime_error("M exceeds " + entry.first +
                               " metadata dimension");
    }
    if (selection.n > module.weight.shape[0]) {
      throw std::runtime_error("N exceeds " + entry.first +
                               " metadata dimension");
    }
    if (selection.k > module.input.shape[1]) {
      throw std::runtime_error("K exceeds " + entry.first +
                               " metadata dimension");
    }
  }
  return selection;
