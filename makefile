...
LLM_P32_REQUIRE_DEFAULT_TRACE_MODEL := y
...
llm_p32_trace_config:
	...
	@if test "$(LLM_P32_REQUIRE_DEFAULT_TRACE_MODEL)" = "y" && grep -q '"profile"' "$(LLM_P32_TRACE_DIR)/metadata.json" && ! grep -Eq '"model"[[:space:]]*:[[:space:]]*"$(LLM_P32_DEFAULT_MODEL_DIR)"' "$(LLM_P32_TRACE_DIR)/metadata.json"; then echo "LLM default trace model does not match selected profile: expected $(LLM_P32_DEFAULT_MODEL_DIR)" >&2; exit 2; fi
